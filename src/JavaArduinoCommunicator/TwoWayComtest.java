...
public class TwoWayComtest implements Runnable {
	private OutputStream out;
	private BufferedReader in;
	
	public TwoWayComtest(String port) {
		super();
		try {
			connect(port);
		}
	...
	private void connect(String portName) throws Exception {
		...
		SerialPort serialPort=((SerialPort)(portIdentifier.open(this.getClass().getName(), 500)));
		...
		System.out.println("Begin Parity Check");
		while(!in.ready()) {
			//System.out.println(in.readLine());
		}
		String str=in.readLine();
		if(str.equals("49")) {
			System.out.println("Parity Check Complete! "+str);
		}
		else {
			System.out.println("Parity Check Failure! "+str);
		}
		*/
		outThread.start();
		return;
	...
	public void writePixel(int val, int r, int g, int b) {
		
	}
	
	public void allOn(int r, int g, int b) {
	...
	public static void main(String[] args) {
		try {
			...
			TwoWayComtest comm=new TwoWayComtest(port);
			comm.connect(port);
		}
		catch(Exception e) {
			e.printStackTrace();
